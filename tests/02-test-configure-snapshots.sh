...
# Updated to use test_* functions approach and leverage the test-utils.sh framework
...
setup() {
    ...
    execCmd "Format device" "mkfs.btrfs -f \"$TARGET_DEVICE\""
...
test_with_user_permissions() {
    ...
    execCmd "Reset filesystem" "mkfs.btrfs -f \"$TARGET_DEVICE\""
    assert "[ $? -eq 0 ]" "Filesystem reset should succeed"
